...
def test_get_event_list__by_user_id(client: TestClient) -> None:
    user_id = 1
    r: requests.Response = client.post("/api/event/", user_id=user_id)
    assert r.status_code == StatusCode.OK
...
def test_get_event(client: TestClient) -> None:
    event_id = 1
    user_id = 1
    r: requests.Response = client.post(f"/api/event/{event_id}", user_id=user_id)
    assert r.status_code == StatusCode.OK
...
def test_delete_event_error_invalid_user(client: TestClient) -> None:
    event_id = 1
    user_id = 2
    r: requests.Response = client.delete(f"/api/event/{event_id}", user_id=user_id)
    assert r.status_code == StatusCode.BadRequest
